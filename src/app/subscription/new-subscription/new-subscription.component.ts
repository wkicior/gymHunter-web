...
import {ClearSelectedSubscriptionIfAny, Subscribe} from "../subscriptions.actions";
...
export class NewSubscriptionComponent implements OnInit {
  ...
  successfulySubscribed$ = this.store.select(createdSubscription).pipe(
    filter(s => s !== null),
  ...
  ngOnInit() {
    this.store.dispatch(new ClearSelectedSubscriptionIfAny());
